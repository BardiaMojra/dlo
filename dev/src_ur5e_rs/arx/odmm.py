...
class dmm:
  ...
  def __init__(self,
    ...
    if name == 'dataset-iphone1_clean':
      ...
      logging.error(lhead+'Err--->> selected dataset not found: '+name+ltail)
  ...
  def format_data(self):
    ...
    if self.name=="dataset-iphone1_clean" and self.ext=='csv':
      ...
      logging.error(lhead+'Err--->> invalid name and/or ext!\n\n', file=sys.stderr)
      exit()
    ''' common df section '''
  ...
  def load_imu_vi_data(self):
    ...
    if np.all(compare == True):
      ...
      logging.error(shead+'timestamp mismatch...'+ltail)
      exit()
  ...
  def load_kitti_set(self):
    ...
    for file in sorted(files):
      ...
      if len(labels) != datum.shape[0]:
        logging.error(lhead+"Err--->> bad datum: "+file+ltail)
        exit()
      if imu_data is None:
...
def plot_quat_vs_quat(quat_A_df,
  ...
  if output_dir is None:
    logging.error(lhead+'Err--->> no output directory assigned for figure: '+figname+' .....\n\n')
...
def plot_df(df:pd.DataFrame,
  ...
  for n, ax in enumerate(axs.flatten()):
    ...
    if col in t_cols:
      ...
      logging.error(lhead+'Err--->> column label missing for figure: '+figname+' .....\n\n')
...
def plot_z_df_vs_x_df_iso(z_df:pd.DataFrame,
  ...
  if len(z_df.columns) != len(x_df.columns):
    logging.error(lhead+'Err--->> in plot_z_df_vs_x_df_iso(), dataframes have UNMATCHED \
      number of columns...'+ltail)
  if labels is None:
    logging.error(lhead+'Err--->> in plot_z_df_vs_x_df_iso(), no df label passed...'+ltail)
  if end is None:
  ...
  for n, ax in enumerate(axs.flatten()):
    ...
    if zcol != xcol:
      ...
      logging.error(lhead+'Err--->> plot_z_df_vs_x_df_iso(): UNMATCHED column LABELS...'+ltail)
    else:
...
def plot_z_df_vs_x_df_grp(z_df:pd.DataFrame,
  ...
  # check
  if len(z_df.columns) != len(x_df.columns):
    logging.error(lhead+'Err--->> in plot_z_df_vs_x_df_grp(), dataframes have UNMATCHED \
      number of columns...'+ltail)
  if labels is None:
    logging.error(lhead+'Err--->> in plot_z_df_vs_x_df_grp(), no df label passed...'+ltail)
  if end is None:
    end = min(z_df.index.size,x_df.index.size)-1
  rows = int(3)
  cols = int(2)
  ...
  for n, ax in enumerate(axs.flatten()):
    ...
    if zcol in t_cols:
      if n%2 == 1:  # z
        ...
        logging.error(lhead+'Err--->> must be unreachable...'+ltail)
      if t_df_min is None or t_df_max is None:
        t_df_min =min(min(z_df[z_df.columns[z_df.columns.isin(t_cols)]].min(skipna=True)),\
                      min(x_df[x_df.columns[x_df.columns.isin(t_cols)]].min(skipna=True)))
        t_df_max =max(max(z_df[z_df.columns[z_df.columns.isin(t_cols)]].max(skipna=True)),\
