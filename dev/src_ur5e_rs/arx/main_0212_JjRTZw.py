...
def prt_state(idx, st, idx_wp, move_completed, keep_running, watchdog):
  nprint("idx",idx)
  ...
  nprint("move_completed", move_completed)
  nprint("keep_running", keep_running)
  nprint("watchdog.input_int_register_0", watchdog.input_int_register_0)
...
def main():
  ...
  # cfg.tdtime = datetime.now().strftime("%Y-%m-%d_%H-%M-%S") # time tag # type: ignore
  # if TIMESTAMP_TID:
  ...
  if UR5ROBOT_EN:
    ...
    con_ur5e = rtde.RTDE(cfg.ur_host, cfg.ur_port)
    con_ur5e.connect()
    con_ur5e.get_controller_version()
    # setup recipes
    con_ur5e.send_output_setup(st_labs, st_dtypes, frequency=cfg.ur_freq)
  ...
  try:
    while keep_running:
      ...
      if UR5ROBOT_EN:
        ...
        prt_state(idx, state, idx_wp, move_completed, keep_running, watchdog) # print state to terminal
        dset.log_st(idx, idx_wp, state, None)
       ...
       #robot.check_waypoint(opt_st, rob_st) #todo add robot waypoint handler
        ...
        if move_completed and state.output_int_register_0 == 1: # next waypoint
          nprint("next waypoint")
          move_completed = False
          idx_wp += 1
          new_setp = wpoints[idx_wp]
          list_to_setp(setp, new_setp)
          nprint("new waypoint", new_setp)
          logging.warning("next waypoint: {idx_wp}.")
          con_ur5e.send(setp)
          if idx_wp >= len(wpoints) and cfg._repeat_wps is True:
            nprint("next waypoint>repeat_wps")
            logging.warning("next waypoint>repeat_wps")
            idx_wp = 0
            logging.info("restart at waypoint 0.")
          elif idx_wp > len(wpoints) and cfg._repeat_wps is False:
            nprint("next waypoint>final_wp_stop")
            logging.warning("next waypoint>final_wp_stop")
          ...
          logging.warning("move to confirmed pose: ", state.target_q)
          move_completed = True
          watchdog.input_int_register_0 = 0
        elif ((tnow-t0) >= cfg.runtime_max): # timeout
          nprint("next waypoint>timeout_stop")
          logging.warning("next waypoint>timeout_stop")
