...
class rsDCamera_cls:
  ...
  def init_device(self): # config and enable streams
    ...
    if self.device_product_line == 'L500': # None BGR format used for cv
      self.config.enable_stream(rs.stream.color, 960, 540, rs.format.bgr8, self.freq)
    else:
